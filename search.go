...
func (s *Search) ybw(brd *Board, stk Stack, alpha, beta, depth, ply, nodeType,
	...
	// if the isSp flag is set, a worker has just been assigned to this split point.
...
searchMoves:
	...
	for m, stage := selector.Next(recycler, spType); m != NO_MOVE; m, stage = selector.Next(recycler, spType) {
		...
		stk[ply+1].inCheck = givesCheck // avoid having to recalculate inCheck at beginning of search.
